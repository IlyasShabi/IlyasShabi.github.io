...
(function ($) {
	...
	$(document).ready(function () {
		...
		$(function () {
			$("#join-now-anchor").on('click', function (event) {
				...
				if (name == "") {
					...
					$(".ds-join-now-message").stop(true).html('<i class="fa fa-warning"></i> Store name is required.');
					...
					if (name.includes('myshopify')) {
						const str = name.split(".myshopi");
						name = str[0];
					}
					// $.ajax({
					// 	type: "POST",
		...
		$("header").backstretch("img/background.jpg", { width: 10 });
